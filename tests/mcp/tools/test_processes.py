...
what is *ours*: the output-dict contract, pk/uuid acceptance, the error
envelope, and the ``%process%`` filter plus state-from-attributes assembly.
See ``tests/conftest.py`` for the real, session-scoped process fixtures.
"""
...
def test_get_process_status_not_found() -> None:
    """An unknown identifier yields an error envelope, not a raised exception."""
    result = get_process_status(987654321)
    assert "error" in result
    assert "987654321" in result["error"]
